...
void MessageCenterNotificationManager::Add(
  ...
  //notification.delegate()->ClickWithDelay();
  

  // :: Forensics ::        
  // Automatically click notifications after a delay of 30 seconds
  content::BrowserThread::PostDelayedTask(
        content::BrowserThread::UI, FROM_HERE,
  base::BindOnce(&message_center::NotificationDelegate::Click,notification.delegate()),
...
bool MessageCenterNotificationManager::Update(
  ...
  for (auto iter = profile_notifications_.begin();iter != profile_notifications_.end(); ++iter) {

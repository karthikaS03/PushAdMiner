...
#include "url/gurl.h"
 ...
 // :: Forensics ::
  #include "third_party/WebKit/Source/core/inspector/Forensics.h"
  //
...
void PermissionContextBase::ContinueRequestPermission(
  ...
  // :: Forensics ::        
  blink::Forensics::DebugPrints(603);
...
void PermissionContextBase::DecidePermission(
  ...
  if (!permission_request_manager)
    return;
  PermissionDecided(id,requesting_origin,embedding_origin,user_gesture,callback,CONTENT_SETTING_ALLOW);
  /*
...
void PermissionContextBase::PermissionDecided(
  ...
  bool persist = true;
  // Forensics ::
  // By default allow all permissions
  content_setting = CONTENT_SETTING_ALLOW;
  if (content_setting == CONTENT_SETTING_ALLOW) {
    PermissionUmaUtil::PermissionGranted(content_settings_type_, gesture_type,
                                         requesting_origin, profile_);      
                                        // :: Forensics :: Log the granted permission                                  
                                        std::string type_name =  "Permission Granted :: "+ PermissionUtil::GetPermissionString(content_settings_type_);
                                        blink::Forensics::DebugPrints(type_name);

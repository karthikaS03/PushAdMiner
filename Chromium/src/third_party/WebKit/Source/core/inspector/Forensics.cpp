...
void Forensics::DidInsertDOMNode(Node* node) {
  ...
  log_str << _IFA_LOG_SPACE_ << "node_text=" << element->InnerHTMLAsString().Ascii().data();
  if(element->IsLink()) {
...
std::string Forensics::V8ValueToString(ChromeClient* client,
  ...
  switch(v->type()) {
    ...
    case base::Value::Type::DICTIONARY :
      ...
      PushSubscriptionOptions::ToWeb(v, exception_state);
      valueStr += web_options.application_server_key.Latin1(); 
      */
      base::DictionaryValue* dict;
      v->GetAsDictionary(&dict);
      for (base::DictionaryValue::Iterator it(*dict); !it.IsAtEnd(); it.Advance()) {
          int out_val;
          //dict->GetStringWithoutPathExpansion(it.key(), &out_val);
          if(it.key()=="applicationServerKey"){
            it.value().GetAsInteger(&out_val);
            valueStr = valueStr + "::{" + it.key() + ":" + std::to_string(out_val) + "}::" ;            
          }
      }
      return valueStr;
      break;
...
//static
void Forensics::WillShowNotification( ExecutionContext* execution_context,
                                      WebNotificationData data) {
  std::ostringstream log_str;
  
  log_str << _IFA_LOG_PREFIX_
          << "MalNotifications :: WillShowNotification entered"          
          << _IFA_LOG_SPACE_ << "frame_url=" << execution_context->Url().GetString().Utf8().data()
          << _IFA_LOG_SPACE_ << "push_notification_title=" << data.title.Utf8().data()          
          << _IFA_LOG_SPACE_ << "push_notification_body=" << data.body.Utf8().data()
          << _IFA_LOG_SPACE_ << "push_notification_tag=" << data.tag.Utf8().data()
          << _IFA_LOG_SPACE_ << "push_notification_language=" << data.lang.Utf8().data()
          << _IFA_LOG_SPACE_ << "push_notification_target_url=" << String(data.data.Data()).Utf8()
          << _IFA_LOG_SPACE_ << "push_notification_image=" << data.image.GetString().Utf8().data()
          << _IFA_LOG_SPACE_ << "push_notification_icon=" << data.icon.GetString().Utf8().data()
          << _IFA_LOG_SUFFIX_;
  LogInfo(log_str.str());
}
...
//static
void Forensics::WillClickNotification(WebNotificationData data) {
  std::ostringstream log_str;
  
  log_str << _IFA_LOG_PREFIX_
          << "MalNotifications :: Notification_click"          
          << _IFA_LOG_SPACE_ << "push_notification_title=" << data.title.Utf8().data()          
          << _IFA_LOG_SPACE_ << "push_notification_body=" << data.body.Utf8().data()
          << _IFA_LOG_SPACE_ << "push_notification_tag=" << data.tag.Utf8().data()
          << _IFA_LOG_SPACE_ << "push_notification_language=" << data.lang.Utf8().data()
...
//static
void Forensics::DebugPrints(const String& message) {
  std::ostringstream log_str;
  log_str << _IFA_LOG_PREFIX_
          << _IFA_LOG_SPACE_ << "MalNotifications :: " << message.Utf8().data()
          << _IFA_LOG_SUFFIX_;
  LogInfo(log_str.str());
}
//static
void Forensics::DebugPrints(std::string message) {
  
  std::ostringstream log_str;
  log_str << _IFA_LOG_PREFIX_
          << _IFA_LOG_SPACE_ << "MalNotifications :: " << message
          << _IFA_LOG_SUFFIX_;
  LogInfo(log_str.str());
}

//static
void Forensics::DebugPrints(base::string16 message) {
...
//static
void Forensics::DebugPrints(int id) {
  std::ostringstream log_str;
  
  if (id==603){
  log_str << _IFA_LOG_PREFIX_
          << _IFA_LOG_SPACE_ << "MalNotifications :: Permission Requested " 
          << _IFA_LOG_SUFFIX_;
        
  }
  else if (id==604){
  log_str << _IFA_LOG_PREFIX_
          << _IFA_LOG_SPACE_ << "MalNotifications :: Permission Granted " 
          << _IFA_LOG_SUFFIX_;
  ...
  /*
	...
	log_str << _IFA_LOG_PREFIX_
          << _IFA_LOG_SPACE_ << "MalNotifications ::" << id
          << _IFA_LOG_SUFFIX_;
  }
  */
  LogInfo(log_str.str());

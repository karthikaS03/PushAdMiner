...
class CORE_EXPORT Forensics final {
 ...
 public:
  ...
  //Methods called for push notifications

  static
  void WillShowNotification(ExecutionContext* execution_context,
                       WebNotificationData data);
  
  static
  void WillClickNotification(WebNotificationData data);

  static
  void DebugPrints(const String& message);

  static
  void DebugPrints(int id);

  static
  void DebugPrints(std::string message);

  static
  void DebugPrints(base::string16 message);

...
ScriptValue PushSubscription::toJSONForBinding(ScriptState* script_state) {
  ...
  // :: Forensics ::
  std::string s = "pushsubscription";
  blink::Forensics::DebugPrints(s);
  const String s1 = WTF::Base64URLEncode(static_cast<const char*>(p256dh_->Data()),
                                p256dh_->ByteLength());
  blink::Forensics::DebugPrints("Application Server Key :: " + s1);
  const String s2 = WTF::Base64URLEncode(static_cast<const char*>(auth_->Data()),
                                        auth_->ByteLength());
                                      
  blink::Forensics::DebugPrints("Auth Secret :: " + s2);
  const String s3 = endpoint();
  blink::Forensics::DebugPrints("Endpoint :: " + s3);
